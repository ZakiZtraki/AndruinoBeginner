...
A breadboard is a reusable prototyping platform that lets you build circuits without soldering. Its holes are organized internally by metal strips that connect rows or columns together ([SparkFun breadboard tutorial](https://learn.sparkfun.com/tutorials/how-to-use-a-breadboard/all)). Key parts include:

- **Tie points:** holes where you insert wires or component leads.
- **Power rails:** long columns on the sides used to distribute power and ground ([SparkFun breadboard tutorial](https://learn.sparkfun.com/tutorials/how-to-use-a-breadboard/all)).
- **Isolated rows and columns:** grids where most components are placed.

When powering a breadboard, connect your power supply’s positive terminal to a red power rail and the negative terminal to the blue/black rail. Always verify connections with a multimeter (measure the voltage across the rails) before connecting any components ([SparkFun breadboard tutorial](https://learn.sparkfun.com/tutorials/how-to-use-a-breadboard/all)).

> *Tip:* If you’re a visual learner, open the recommended article’s labelled breadboard diagram. It illustrates how the power rails and tie points are internally connected and can help cement your understanding ([SparkFun breadboard tutorial](https://learn.sparkfun.com/tutorials/how-to-use-a-breadboard/all)).
...
Read the *Wiring Up Your First Components with a Breadboard* guide (Blues University, 2024). Focus on the sections explaining breadboard layout and how to connect the power rails. If you need an alternate, see the [SparkFun breadboard tutorial](https://learn.sparkfun.com/tutorials/how-to-use-a-breadboard/all).
...
- **USB port and power jack** for programming and power.
- **Replaceable ATmega328P chip**.
- A recommended operating voltage range of **7–12 V** when using an external supply.

Source: [Arduino Uno Rev3 tech specs](https://store.arduino.cc/products/arduino-uno-rev3).
...
- **Use the correct input voltage.** While the Uno can accept 6–20 V, it operates safest at **7–12 V**. Avoid applying >20 V to the power jack or >5 V to the 5 V pin ([Arduino Uno Rev3 tech specs](https://store.arduino.cc/products/arduino-uno-rev3)).
- **Don’t draw too much current from a pin.** Each I/O pin is rated for around **20 mA**; drawing more can overheat the pin ([Arduino Uno Rev3 tech specs](https://store.arduino.cc/products/arduino-uno-rev3)).
- **Avoid short circuits.** Connecting power pins directly to ground or connecting two pins set to opposite logic levels can cause large currents that damage the board ([short circuit overview](https://en.wikipedia.org/wiki/Short_circuit)).
- **Protect inductive loads.** Motors and relays can feed voltage spikes back into the board when switched off. Use a flyback diode or motor driver to prevent damage ([SparkFun transistor tutorial](https://learn.sparkfun.com/tutorials/transistors/all)).
- **Respect 3.3 V devices.** Some sensors (especially those used later with the ESP32) operate at 3.3 V. Connecting them directly to 5 V may permanently damage them ([ESP32-WROOM-32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32-wroom-32_datasheet_en.pdf)).

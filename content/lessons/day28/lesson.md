...
2. **Read multiple sensors on the Arduino** and transmit their data to the ESP32 via UART using a safe voltage‑shifting interface.
3. **Serve the sensor data on a web page and publish it via MQTT** using the ESP32’s networking capabilities.
...
Connecting two different microcontrollers requires careful attention to voltage levels: the Uno uses 5 V logic and the ESP32 is 3.3 V‑only. As explained earlier, you must use a bidirectional level‑shifting circuit. A logic level converter has separate high‑voltage (HV) and low‑voltage (LV) pins; **power the HV side at 5 V and the LV side at 3.3 V**. You then route the Uno’s TX through a channel to drop it to 3.3 V before it reaches the ESP32, while the ESP32’s TX can be connected directly to the Uno’s RX because 3.3 V is seen as a logic high by the Uno ([Arduino Uno Rev3 tech specs](https://store.arduino.cc/products/arduino-uno-rev3), [ESP32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf), [SparkFun level shifter guide](https://learn.sparkfun.com/tutorials/bi-directional-logic-level-converter-hookup-guide/all), [ATmega328P datasheet](https://ww1.microchip.com/downloads/en/DeviceDoc/ATmega328P-Data-Sheet-DS-DS40002061B.pdf)).

In this project the Arduino continuously reads a DHT22 temperature/humidity sensor, a resistive water‑level sensor and a PIR motion sensor. It packages these values into a comma‑separated string and sends them to the ESP32 via serial. The ESP32 connects to your Wi‑Fi network, hosts a simple HTTP server and an MQTT client. It parses the incoming serial message, updates the web page and publishes the values to MQTT topics. It also listens for commands via HTTP form submission or MQTT subscription to turn a relay or servo on the Arduino on or off.
...
* Software: Arduino IDE with ESP32 support installed (Day 22), PubSubClient library (Day 25), your Wi‑Fi credentials and MQTT broker address (you can use public test brokers like `broker.hivemq.com` [HiveMQ public broker](https://www.hivemq.com/public-mqtt-broker/)).
...
1. **DHT22**: Connect its VCC to 5 V, GND to GND, and Data to digital pin 7. Add a 10 kΩ pull‑up resistor between Data and VCC as recommended for reliable reading ([Adafruit DHT guide](https://learn.adafruit.com/dht)).
2. **Water level sensor**: Use the resistive sensor module. Its analog output varies with immersion; connect `AO` to A0 on the Uno. To reduce corrosion, power the sensor via an Arduino digital pin (set it HIGH only when taking a reading) ([Grove Water Sensor](https://wiki.seeedstudio.com/Grove-Water_Sensor/), [Electrolysis](https://en.wikipedia.org/wiki/Electrolysis)).
3. **PIR sensor**: Connect VCC to 5 V, GND to GND and `OUT` to digital pin 8. Adjust sensitivity and delay as described previously ([HC‑SR501 guide](https://www.makerguides.com/hc-sr501-arduino-tutorial/)).
4. **Optional actuator**: Connect a relay module or a servo to digital pin 9. If using a servo, power it from the 5 V rail and remember that it can draw hundreds of milliamps when moving ([Hobby servo guide](https://learn.sparkfun.com/tutorials/hobby-servo-tutorial/all)).

### 1.2 Serial link between Uno and ESP32

1. Connect the Uno’s **TX** pin (digital 1, or the TX pin of your `SoftwareSerial`) to a **level‑shifter HV → LV channel**. Connect the corresponding LV pin to the ESP32’s RX pin (e.g., GPIO 16). Power the level shifter’s HV pins with 5 V and its LV pins with 3.3 V.
2. Connect the ESP32’s **TX** pin (GPIO 17) directly to the Uno’s RX pin (digital 0 or your `SoftwareSerial` RX pin). The Uno accepts 3.3 V as HIGH.
3. Connect the grounds of both boards together.

### 1.3 ESP32 network and MQTT wiring

The ESP32 will use its built‑in Wi‑Fi. If you haven’t already, connect it to your router using `WiFi.begin(ssid, password)` and wait for `WiFi.status()` to be `WL_CONNECTED`. Install the PubSubClient library as described on Day 25. Ensure the ESP32’s 3.3 V regulator can deliver enough current (~250 mA during Wi‑Fi transmissions); power it via USB or a stable 5 V source ([ESP32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf)).
...
int readWaterLevel() {
  ...
  digitalWrite(WATER_POWER_PIN, LOW);        // power off to reduce corrosion
...
Source: [Arduino‑ESP32 Serial](https://docs.espressif.com/projects/arduino-esp32/en/latest/api/serial.html).
...
void setup() {
  ...
  client.setServer(mqtt_server, mqtt_port);        // set broker address
...
Source: [Arduino‑ESP32 WiFi class](https://docs.espressif.com/projects/arduino-esp32/en/latest/api/wifi.html), [Arduino‑ESP32 WebServer](https://docs.espressif.com/projects/arduino-esp32/en/latest/api/webserver.html), [PubSubClient API](https://pubsubclient.knolleary.net/api).
...
This code connects to Wi‑Fi and MQTT, reads lines from the Arduino, and updates variables. It publishes a JSON string containing all sensor readings to the `hybrid/sensors` topic and listens for commands on the `hybrid/cmd` topic. The web server presents a simple dashboard and a link to toggle the actuator. You can enhance the interface with CSS or JavaScript for auto‑refresh.
...
* **No serial communication** – Check that TX/RX lines are crossed and that the level converter is powered properly. Ensure both boards use the same baud rate and that you’re not sending newline characters from one board and expecting them on the other.
* **Incorrect sensor values** – Confirm wiring and calibrate sensor ranges. DHT22 should return values within –40 to 80 °C and 0 to 100 % RH ([DHT22 datasheet](https://cdn-shop.adafruit.com/datasheets/DHT22.pdf)). Water sensor readings depend on your calibration; adjust the mapping in the Arduino sketch.
* **ESP32 doesn’t connect to Wi‑Fi** – Verify SSID/password and that your router is within range. The code waits until `WiFi.status()` returns `WL_CONNECTED`.
* **MQTT problems** – Ensure the broker address is reachable and that the port is open. Use a unique client ID and check the reconnection loop.
...
In this hybrid project you built a two‑board system that combines the sensor‑handling strengths of the Arduino with the networking abilities of the ESP32. The Uno reads a DHT22, a water level sensor and a PIR sensor, powers the water sensor only when needed to reduce corrosion and packages the results into a serial message. A level‑shifter ensures that the 5 V serial output is safely dropped to 3.3 V before reaching the ESP32, while the ESP32’s 3.3 V TX line can drive the Uno directly. The ESP32 connects to Wi‑Fi, runs a web server and an MQTT client, parses incoming data and publishes it as JSON. A simple dashboard displays live values and toggles an actuator. This foundation can be expanded with more sensors, persistent settings and secure communication, demonstrating how two microcontrollers can collaborate to build robust IoT systems.

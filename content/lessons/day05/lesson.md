...
Arduino boards like the Uno include a multichannel 10‑bit **analog‑to‑digital converter (ADC)**.  When you call `analogRead(pin)`, the ADC measures the voltage present on that analog pin (relative to the board’s reference voltage) and converts it into an integer between **0 and 1023**【538618890236897†screenshot】.  On a 5 V Uno, 0 represents 0 V and 1023 represents 5 V, so each step corresponds to **about 4.9 mV**【538618890236897†screenshot】.  Boards with different reference voltages (e.g., 3.3 V or a custom reference set with `analogReference()`) will map the range accordingly.

### How a potentiometer becomes a voltage divider

A **potentiometer** has three terminals: two outer pins connected to a fixed resistance and a **center “wiper”** that moves along the resistive track.  By connecting the outer pins to **5 V and ground** and the center wiper to an analog input, you create a **voltage divider**.  Turning the shaft changes the ratio of resistance on either side of the wiper and therefore the voltage on the center pin.  If the wiper is fully toward the 5 V end, the voltage at the wiper is close to 5 V and `analogRead()` returns a value near 1023; if it’s fully toward ground, `analogRead()` returns a value near 0【938241268246697†screenshot】.
...
The Arduino documentation’s **Read Analog Voltage** example demonstrates this conversion and explains that the ADC returns 0 when 0 V is applied and 1023 when 5 V is applied【938241268246697†screenshot】.  The example also highlights that boards using 3.3 V must adjust the numerator accordingly【731371549656911†screenshot】.

### Generating analog‑like output with PWM

Microcontrollers cannot output true analog voltages directly on most pins, but they can produce a **pulse‑width modulated (PWM) signal** that approximates an analog level.  When you call `analogWrite(pin, value)`, the pin generates a square wave whose **duty cycle** (percentage of the time it is HIGH) corresponds to the **value** between **0 (always off) and 255 (always on)**【968801034431758†screenshot】.  The Arduino core manages this for you using hardware timers on specific PWM pins (3, 5, 6, 9, 10 and 11 on the Uno)【453991055661574†L32-L37】.  After you call `analogWrite()`, the PWM signal continues until another call to `analogWrite()` or a call to `digitalRead()`/`digitalWrite()` on the same pin【687279365115582†screenshot】.  Note that only **four different PWM outputs can be active simultaneously**; enabling more will stop the sketch【987426609487138†screenshot】.
...
3. Connect the **middle pin (wiper)** to **analog pin A0**.  The Arduino’s **Read Analog Voltage** example notes that connecting the three wires this way allows the ADC to measure a voltage between 0 V and 5 V as you turn the knob【775145145829875†screenshot】.  Turning the potentiometer changes the ratio of resistance on either side of the wiper and therefore the voltage at the center pin【938241268246697†screenshot】.
...
2. Connect one end of a **220 Ω resistor** to digital pin 9 (this is one of the Uno’s PWM pins).  **Connect the other end of the resistor to the LED’s anode**, and connect the LED’s cathode to **GND**.  The Arduino example for **Analog In, Out Serial** uses this arrangement and notes that the long leg of the LED connects to the resistor and the shorter leg goes to ground【742809433890967†screenshot】.

> **Checkpoint:** With the potentiometer wired from Step 1 and the LED/resistor connected, your circuit should match the diagram in the Arduino example【742809433890967†screenshot】.  Double‑check each connection before powering the board.
...
void loop() {
  ...
  float voltage = sensorValue * (5.0 / 1023.0);  // convert to volts【731371549656911†screenshot】
...
Upload this sketch.  As you turn the knob, you should see the raw value and calculated voltage change smoothly.  If you’re using a 3.3 V board, replace `5.0` with `3.3` as the documentation suggests【731371549656911†screenshot】.

## Step 4 – Map the value to PWM range

Because `analogWrite()` accepts values between **0 and 255**【968801034431758†screenshot】, you need to scale the 0–1023 range down.  The Arduino’s **Analog In, Out Serial** example shows how to use the `map()` function:
...
The example explains that the Uno’s `analogRead()` returns 0–1023 while `analogWrite()` expects 0–255, so the value must be converted before dimming the LED【148329490764575†screenshot】.  The `map()` function takes five arguments: the value to remap, the low and high limits of the original range, and the low and high limits of the target range【148329490764575†screenshot】.  An alternative is to divide the sensor value by 4 (since 1023 ÷ 4 ≈ 255).
...
> **Why 0–255?** The PWM hardware on most AVR‑based Arduinos has an **8‑bit resolution**, giving 256 discrete duty‑cycle levels (0–255).  The `analogWrite()` function produces a square wave whose duty cycle corresponds to this value【968801034431758†screenshot】.  The documentation warns that only four PWM outputs can be active simultaneously; enabling more will stop the sketch【987426609487138†screenshot】.
...
- **3.3 V boards:** Adjust the voltage calculation and mapping ranges to suit the board’s reference voltage【731371549656911†screenshot】.

## Summary

In this lesson you learned that the **ADC on the Arduino Uno is 10 bits**, mapping analog voltages between 0 V and the reference voltage to integers **0–1023**【538618890236897†screenshot】.  By wiring a potentiometer as a voltage divider and reading it with `analogRead()`, you can obtain a continuous range of values.  To control the brightness of an LED, you convert the 10‑bit reading to an **8‑bit PWM value** (0–255) using the `map()` function【148329490764575†screenshot】 and write it to a PWM‑capable pin with `analogWrite()`【968801034431758†screenshot】.  PWM produces a square wave whose duty cycle approximates an analog voltage, letting you dim LEDs, control motors or vary sound pitch.  With this foundation, you’re ready to explore other analog sensors and build responsive, analog‑aware projects.
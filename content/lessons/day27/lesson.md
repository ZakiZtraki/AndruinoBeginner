...
* **UART (Universal Asynchronous Receiver‑Transmitter)** – also called “serial”. It uses a TX (transmit) line, an RX (receive) line and a common ground. Devices agree on a baud rate but don’t share a clock. UART is easy to implement and perfect for streaming ASCII or binary data. When connecting two boards, **the TX of one must go to the RX of the other, and the grounds must be connected** ([ATmega328P datasheet](https://ww1.microchip.com/downloads/en/DeviceDoc/ATmega328P-Data-Sheet-DS-DS40002061B.pdf)).
* **I²C (Inter‑Integrated Circuit)** – a synchronous bus that uses open‑drain SDA (data) and SCL (clock) lines pulled up by resistors. Multiple devices share the same two wires; each device has a 7‑bit address and the controller (master) initiates communication. **Mixing 5 V and 3.3 V devices on the same I²C bus requires a bidirectional level converter**, because the SDA and SCL lines are pulled up to a specific voltage and data flows both ways ([I²C‑bus specification](https://www.nxp.com/docs/en/user-guide/UM10204.pdf)).

### Voltage considerations

The Uno’s I/O pins operate at 5 V, whereas the ESP32’s pins run at 3.3 V. Driving the ESP32’s pins with 5 V will damage them ([Arduino Uno Rev3 tech specs](https://store.arduino.cc/products/arduino-uno-rev3), [ESP32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf)). A bi‑directional logic‑level converter (for example the SparkFun BSS138‑based board) provides separate high‑voltage (HV) and low‑voltage (LV) supplies. When interfacing a 5 V device to a 3.3 V device you **power the HV side with 5 V and the LV side with 3.3 V** ([SparkFun logic‑level converter](https://learn.sparkfun.com/tutorials/bi-directional-logic-level-converter-hookup-guide/all)). The converter has independent channels; a signal applied to the low side is shifted up to the high side and vice versa. Without a dedicated converter you can also build a one‑directional voltage divider to drop the Arduino’s TX signal (for example, 2 kΩ/1 kΩ to scale 5 V down to ~3.3 V). Going the other direction is usually safe because the Uno recognises 3.3 V as a logic high ([ATmega328P datasheet](https://ww1.microchip.com/downloads/en/DeviceDoc/ATmega328P-Data-Sheet-DS-DS40002061B.pdf)).

For I²C, the SDA and SCL lines are open‑drain outputs that require pull‑up resistors to the supply voltage. Typical values are **4.7 kΩ – 10 kΩ**. When mixing 5 V and 3.3 V devices you **must place the level converter between the two sets of pull‑ups** ([I²C‑bus specification](https://www.nxp.com/docs/en/user-guide/UM10204.pdf)).
...
2. **Transmit from Uno to ESP32**: On the Uno, choose the pin that will transmit data. If you don’t need the Serial Monitor on the Uno, you can use its hardware `Serial` on pins 0 (RX) and 1 (TX). Otherwise, use the `SoftwareSerial` library on two other pins (e.g., 2 = RX and 3 = TX) ([SoftwareSerial reference](https://www.arduino.cc/en/Reference/softwareSerial)). Connect the Uno’s TX pin to **HV1** of the level converter, and connect **LV1** to the ESP32’s RX pin (e.g., GPIO16). The converter drops the 5 V TX signal down to 3.3 V.
...
4. **Verify connections**: Remember to **cross the TX and RX lines**—**TX of one board goes to RX of the other**, and the grounds must be connected. If you choose other pins on the ESP32, define them when calling `HardwareSerial.begin()`.
...
Upload the following sketch to the ESP32. It uses UART2 (Serial2) with pins 16 (RX) and 17 (TX) at 9600 baud to read and print incoming data. We specify the pins when we call `Serial2.begin()` ([Arduino‑ESP32 Serial](https://docs.espressif.com/projects/arduino-esp32/en/latest/api/serial.html)):
...
1. Use a **bi‑directional level shifter**. Connect the Uno’s SDA (A4) and SCL (A5) to the HV side of two channels, and the ESP32’s SDA (default GPIO 21) and SCL (default GPIO 22) to the LV side. Power the HV side with 5 V and the LV side with 3.3 V. Connect both boards’ grounds ([Arduino Wire](https://www.arduino.cc/en/Reference/Wire), [Arduino‑ESP32 I²C](https://docs.espressif.com/projects/arduino-esp32/en/latest/api/i2c.html)).
2. Ensure there are pull‑up resistors on both sides. Typical values are 4.7 kΩ–10 kΩ. Many I²C breakout boards include pull‑ups, but when you build your own connections you must add them. Keep the total parallel resistance above about 2.2 kΩ ([I²C‑bus specification](https://www.nxp.com/docs/en/user-guide/UM10204.pdf)).
...
4. Assign an unused 7‑bit address (for example `0x42`) to the peripheral and ensure no other device on the bus uses the same address.
...
* **Check the wiring** – Most problems are due to mis‑wired TX/RX lines or missing grounds. Double‑check that TX goes to the opposite board’s RX and that the level shifter HV and LV pins are powered correctly.
* **Use the correct baud rate** – Both boards must use the same baud rate in `begin()`. A mismatch results in garbled characters.
* **Don’t exceed the ESP32’s logic levels** – Always use a level shifter or voltage divider on signals coming from the Uno to the ESP32 ([ESP32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf)).
* **Beware of I²C pull‑ups** – If you mix many breakout boards, the parallel resistance can become too low. Remove unnecessary pull‑ups to keep the effective pull‑up between ~2.2 kΩ and 10 kΩ.
* **Use unique I²C addresses** – Two devices with the same address on the same bus will conflict. For microcontroller‑to‑microcontroller communication, choose an address > 0x08 ([I²C‑bus specification](https://www.nxp.com/docs/en/user-guide/UM10204.pdf)).
...
* **SoftwareSerial limitations** – `SoftwareSerial` can be unreliable at higher baud rates. For robust communication, use hardware serial ports or consider connecting through USB to a computer for debugging ([SoftwareSerial reference](https://www.arduino.cc/en/Reference/softwareSerial)).
